          ...
          switch (hv.layer.id) {
            ...
            default:
              if (hv.layer.id.indexOf('piechart-layer') == 0) {
                return {
                  html: `Beers:<table>${hv.object.beersBrewed.map(bb => createLabelRow(bb)).join('')}</table>`,
                  style: {
                    fontSize: '0.8em'
                  }
                }
...
function createLabelRow(beerBrewed) {
  const colSet = `${beerBrewed.color[0]},${beerBrewed.color[1]},${beerBrewed.color[2]}`
  return `<tr class='beer-row'>
            <td>
              <div class="mud-elevation-0 d-flex justify-center align-center" style="height:24px;">
                <svg class="mud-icon-root mud-svg-icon mud-light-text mud-icon-size-medium" focusable="false" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M0 0h24v24H0z" fill="none"></path>
                  <path d="M12 2C6.47 2 2 6.47 2 12s4.47 10 10 10 10-4.47 10-10S17.53 2 12 2z" fill="rgb(${colSet})"></path>
                </svg>
              </div>
            </td>
